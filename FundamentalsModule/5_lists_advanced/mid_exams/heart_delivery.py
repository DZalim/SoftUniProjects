...
while command != "Love!":
    ...
    if index_to_jump < len(size_of_neighborhood):
        ...
        current_index_of_cupid = 0
        if size_of_neighborhood[current_index_of_cupid] == 0:
            print(f"Place {current_index_of_cupid} already had Valentine's day.")
        else:
            size_of_neighborhood[current_index_of_cupid] -= 2
            if size_of_neighborhood[current_index_of_cupid] == 0:
                print(f"Place {current_index_of_cupid} has Valentine's day.")
    command = input()
